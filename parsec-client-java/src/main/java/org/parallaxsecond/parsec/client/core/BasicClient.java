...
import org.parallaxsecond.parsec.client.exceptions.InvalidProviderException;
import org.parallaxsecond.parsec.client.exceptions.InvalidServiceResponseTypeException;
import org.parallaxsecond.parsec.client.exceptions.MissingParamException;
import org.parallaxsecond.parsec.client.exceptions.NoAuthenticatorException;
import org.parallaxsecond.parsec.client.exceptions.NoProviderException;
import org.parallaxsecond.parsec.client.exceptions.NotFoundException;
 ...
 *
 * <p>
 * The client exposes low-level functionality for using the Parsec service. Below you can see code
 * examples for a few of the operations supported.
 *
 * <p>
...
public class BasicClient {
   ...
   *
   * <p>
   * The client will be initialised with default values obtained from the service for the implicit
   * provider and for application identity.
   *
   * <p>
   ...
   *
   * <p>
   * ```no_run # use std::error::Error; # # fn main() -> Result<(), Box<dyn Error>> { use
  ...
  public static BasicClient naked() {
    return new BasicClient(OperationClient.withDefaults(), new Authentication.None(),
        ProviderId.CORE);
  }
  ...
  public void setDefaultAuth(String appName) {
    ...
    for (NativeResult.ListAuthenticatorsResult.AuthenticatorInfo authenticator : authenticators
        .getAuthenticators()) {
      switch (authenticator.getId()) {
        case DIRECT:
          this.authData = ofNullable(appName).map(Authentication.Direct::new)
        ...
        case JWT_SVID:
          ...
          break;
        default:
          log.warn("Authenticator of type \"{}\" not supported by this client library",
              authenticator.getId());
          continue;
      }
      return;
