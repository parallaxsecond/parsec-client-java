...
public enum ParsecRsaSignature implements ParsecSignatureInfo {
    ...
    SHA512_WITH_RSA("SHA512withRSA", pkcs1WithHash(PsaAlgorithm.Algorithm.Hash.SHA_512),
            () -> MessageDigest.getInstance("SHA-512")),

    SHA256_WITH_RSA("SHA256withRSA", pkcs1WithHash(PsaAlgorithm.Algorithm.Hash.SHA_256),
    ...
    @Getter
    private final AsymmetricSignature parsecAlgorithm;
    @Getter
    ...
    private static AsymmetricSignature pkcs1WithHash(PsaAlgorithm.Algorithm.Hash hash) {
        return AsymmetricSignature.newBuilder()
                .setRsaPkcs1V15Sign(AsymmetricSignature.RsaPkcs1v15Sign.newBuilder()
                        .setHashAlg(
                                AsymmetricSignature.SignHash.newBuilder().setSpecific(hash).build())
                        .build())
                .build();
    }
    ...
    private static AsymmetricSignature pkcs1() {
        return AsymmetricSignature.newBuilder()
                .setRsaPkcs1V15Sign(AsymmetricSignature.RsaPkcs1v15Sign.newBuilder().build())
                .build();
    }
